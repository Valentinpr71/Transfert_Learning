...
from render.testplot import Test_plot

class microgrid_control_gym(gym.Env):
    metadata = {'render.modes': ['human']}
    def __init__(self, data='train', plot_every=10, ppc=None, sell_to_grid=False,hydrogene_storage_penalty=True, independancy=False):
        super(microgrid_control_gym,self).__init__()
        self.ppc_power_constraint=ppc
        #facteur pour pénaliser la vente au grid central
        if independancy:
            self.independancy=-1
        else:
            self.independancy=1
        ...
        if sell_to_grid:
            print("sell to the grid")
        else:
            self.ppc_power_constraint=0
            print("no sell to grid")
        ...
        self.info={"Timestep":[],"Reward":[],"Energy Surplus":[]}
        self.num_episode=0
    ...
    def step(self, action):
        ...
        if (action==0):
            ...
            if self.hydrogen_storage-self.hydrogen_max_power<0: #If not enough storage to discharge at max power:
                true_energy_avail_from_hydrogen=-self.hydrogen_storage*self.hydrogen_PAC_eta
                diff_hydrogen=-self.hydrogen_storage
            else:
        ...
        # Surplus of energy --> load the battery
            if min(1.,self._last_ponctual_observation[0]-Energy_needed_from_battery/self.battery_size*self.battery_eta)==1 & self.sell_to_grid:
            #If the battery is full and there is a surplus of energy, sell it to the grid at 0.5€ per kW. As the (mean of production is 1.5 kW), we define self.ppc_power_constraint as the PCC threshold
                if self.ppc_power_constraint==None:
                    reward += self.independancy*self._last_ponctual_observation[
                    ...
                    if threshold==self.ppc_power_constraint:#If the surplus>PCC max power
                        ...
                        reward+=self.independancy*self.ppc_power_constraint*0.5
                    else:
                        self.reward+=self.independancy*self._last_ponctual_observation[0]*self.battery_size-Energy_needed_from_battery/self.battery_eta
                        self.info["Energy Surplus"].append(0.)
            if not self.sell_to_grid:
    ...
    #     LOOKBACK_WINDOWS_SIZE=24
    #     if mode == 'file':
    #         self.render_to_file(kwargs.get('filename','render.txt'))
    #     elif mode == 'live':
    #         if self.visualization == None:
    #             self.visualization = MicrogridGraph(data,title)
    #         if self.counter>LOOKBACK_WINDOWS_SIZE:
    #             self.visualization.render(self.counter,self.consumption[self.counter-1]-self.production[self.counter-1],
    #                                       self.reward, windows_size=LOOKBACK_WINDOWS_SIZE)
    ...
    def save_plot_every(self):
        ...
        if self.counter-len(self.consumption)+1==0:
            plot=Test_plot(self.info, self.num_episode, hydrogene_storage_penality=self.hydrogen_storage_penalty, ppc=self.ppc_power_constraint, sell_grid=self.sell_to_grid, independancy=self.independancy)
            plot.save()
    ...
    #     file.write(f'Net Demand: {self.consumption[self.counter-1]-self.production[self.counter-1]} (Max net worth: {self.max_net_worth})\n')
