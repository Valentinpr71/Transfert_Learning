...
class Interact():
    ...
    def get_tuples_random(self):
        ...
        # Modif 5 dec 22 pour l'environnement
        env, self.state_dim, self.num_actions = utils.make_env("MicrogridControlGym-v0", self.manager)
        ...
        for j in range(len(Action_random.columns.value_counts())):
            ...
            while not is_done:
                ...
                rewards_random[j][i-1] = reward/self.coeff_norm
    ...
    def get_tuples_rule(self, epsilon_rule):
        ...
        for j in range(duplications):
            ...
            while not is_done:
                ...
                self.replay_buffer.add(state, action, obs, reward/self.coeff_norm, float(is_done), is_done, episode_start)
                episode_start = False
    ...
    def build_buffer(self):
        ...
        if self.frac_pretrain:
            ...
            self.get_tuples_pretrain(nb_episodes_per_agent=int(self.buffer_size*self.frac_pretrain_exp/(len(self.agents)*self.len_episode))+1)
