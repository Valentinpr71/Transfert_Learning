...
parser.add_argument("--hydrogene_storage_penality", help="activate rewards (or penalities) linked to hydrogene storage", const=True, nargs='?', default=True)
...
parser.add_argument("--independancy", help="if True, reward for selling to grid are negative", const=True, nargs='?', default=False)
...
if __name__=="__main__":
    ...
    if independancy == 1:
        independancy = " Grid dependancy penality "

    fname=hydrogene_storage_penality+ppc+sell_grid+independancy
    ...
    len_episode=8760
    num_episode=20
    env=microgrid_control_gym(data="train",independancy=args.independancy, hydrogene_storage_penalty=args.hydrogene_storage_penality, ppc=args.ppc, sell_to_grid=args.sell_to_grid)
